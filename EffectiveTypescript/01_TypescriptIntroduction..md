...
TS의 타입 시스템은 점진적이고 선택적이다. 타입을 조금씩 추가해갈 수 있고, 언제든지 타입 체커를 해제할 수 있기 때문이다. 이 기능들의 핵심은 any 타입이다. TS의 타입 오류는 대개 as any를 추가함으로써 일단 해결할 수 있습니다. 하지만 이러한 any는 많은 위험성을 가지고 있고 TS로 인한 수많은 장점을 누릴 수 없게 합니다.

- any에는 타입 안정성이 없습니다

as any를 사용하게 되면 변수는 string일 수도 있고, number일 수도 있습니다. 타입 체커는 그저 선언에 따라서 추론을 할 뿐 타입을 보장할 수 없게 됩니다.

- any는 함수 시그니처를 무시해 버립니다.

함수를 작성할 때는 시그니처를 명시해야 하고 이를 통해 호출하는 쪽은 약속된 타입의 입력을 제공하고 함수는 약속된 타입의 출력을 반환하게 됩니다. 그러나 any 타입은 이러한 약속을 지키지 않습니다.

- any 타입에는 언어 서비스가 적용되지 않습니다.

TS 언어 서비스는 자동 완성 기능을 제공합니다. 하지만 이는 타입이 존재할 때만 제공됩니다. 타입이 없다면(any 타입이라면 아무런 도움을 받을 수 없습니다.

- any 타입은 타입 설계를 감춰버립니다.

애플리케이션 상태 같은 객체를 정의하는 일은 복잡한 일이고 수많은 속성의 타입을 필요로 합니다. 이러한 일에 any 타입을 사용한다면 일을 간단히 끝내버릴 수 있지만 별로 도움이 되지 않을 겁니다. 상태 객체의 설계를 감춰버리기 떄문입니다. any 타입을 사용한다면 타입 설계가 불분명해져 설계가 어떻게 되어 있는지 알 수 없습니다. 동료가 코드를 검토할 일이 생긴다면 동료는 코드를 재구성해봐야할 것입니다. 명료한 설계를 위해서는 타입을 일일이 작성하는 것이 좋습니다.
